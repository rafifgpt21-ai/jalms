...
export function StudentDashboardView({
    ...
    // Update time and next class logic
    useEffect(() => {
        const timer = setInterval(() => setCurrentTime(new Date()), 60000) // Update every minute

        // Find next class logic
        // Assuming periods map to specific times. 
        // For simplicity in this demo, let's assume standard times or just find first future period.
        // If we don't have distinct period times in DB, we rely on period index relative to now? 
        // Let's approximate: Period 1 = 07:00, Period 2 = 08:30 etc.
        // Ideally this comes from a helper or DB. 
        // For now, let's just show the first class in the schedule list that hasn't "passed" based on index/ordering if we assume schedule is chronologically ordered for today.

        if (schedule && schedule.length > 0) {
            // Simple logic: Just grab the first one for "Up Next" if it's start of day, 
            // In a real app we'd compare against current time. 
            // Let's just pick the first one for visual demo or index 0.
    ...
    return (
        <div className="space-y-8 p-1 sm:p-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 auto-rows-min">
                ...
                {/* Quick Stats: Attendance or Grades */}
                <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                ...
                >
                    <div style={{ backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)' }} className="bg-white/60 dark:bg-slate-900/40 border border-white/20 dark:border-white/5 rounded-3xl p-6 h-full flex flex-col justify-between shadow-lg shadow-slate-200/50 dark:shadow-none hover:bg-white/70 transition-colors">
                        ...
                        <div>
                            {/* Placeholder for average or just latest grade */}
                            <div className="text-4xl font-heading font-bold text-slate-800 dark:text-slate-100">
