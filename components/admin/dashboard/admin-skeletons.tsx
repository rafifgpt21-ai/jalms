...
export function RecentLoginSkeleton() {
  return <WorkspacePanel className="overflow-hidden" aria-label="Loading recent login activity" aria-busy="true"><HeaderSkeleton /><div className="divide-y">{Array.from({ length: 5 }, (_, index) => <div key={index} className="flex h-14 items-center gap-3 px-4"><Skeleton className="size-8 rounded-full" /><div className="flex-1 space-y-2"><Skeleton className="h-4 w-32" /><Skeleton className="h-3 w-48" /></div><Skeleton className="h-3 w-24" /></div>)}</div></WorkspacePanel>
}
