...
function parseCursor(cursor: CourseChatCursor | undefined) {
  if (!cursor || !/^[a-f\d]{24}$/i.test(cursor.id)) return null
...
export async function sendCourseChatMessage(courseId: string, rawContent: string) {
  ...
  if (isPusherConfigured()) {
    try {
      await getPusherServer().trigger(courseChatChannel(courseId), COURSE_CHAT_EVENT, { messageId: message.id, senderId: access.userId })
    } catch (error) {
      console.error("Failed to publish course chat message", { courseId, messageId: message.id, error })
    }
  } else {
    console.error("Course chat message persisted, but Pusher is not configured", { courseId, messageId: message.id })
  }
